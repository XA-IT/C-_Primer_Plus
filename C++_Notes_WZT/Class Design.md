...
#### 14.9 重载, 类型转换与运算符
**类类型转换** 由转换构造函数与类型转换运算符共同定义
##### 类型转换运算符
- 类型转换函数: `operator type() const;` 
  - 必须是成员函数, 没有显式返回类型, 无形参, 只要type可以作为返回值就可以定义, 将类类型转换为type类型
    ```cpp
    class SmallInt {
    public:
      SmallInt(int i = 0): val(i) {}
      operator int() const { return val; }  //将SmallInt 转换为int(type即为声明)
    private:
      size_t val;
    }
    ``` 
...
- 继承
  ...
  - 对于需要派生类各自定义版本的函数需要被声明为**虚函数**
    - 重新定义的虚函数需要在派生类中声明, C++11 允许使用派生类的成员函数改写基类的虚函数, 加上`override`
- 动态绑定(运行时绑定)
...
#### 15.2定义基类与派生类
##### 定义基类
- 派生类通过对操作的重新定义**覆盖**(override)原来的定义
- 通过`protected`来使派生类访问基类的成员, 这类成员外部无法访问

##### 定义派生类
- 使用 类派生列表 来指出所继承的基类
  - 必须要将需要覆盖的成员 重新声明
- 派生类新定义的成员与从基类继承的子对象不一定是连续存储的
- 派生类构造函数
  - 派生类构造时首先构造基类的部分, 需要调用基类的构造函数构造子对象
  - 遵循基类的接口, 避免在派生类中直接初始化基类的成员
- 被用作基类的类必须已经定义
- C++ 11: `final`跟在类名后 阻止继承的发生
##### 类型转换与继承
- 注意: 基类的指针或引用可以绑定在派生类上
  - 派生类到基类的类型转换
- 静态类型与动态类型
  - 仅对于**指针或引用**, 会在运行时确定的为动态类型
- 不能从基类隐式转换为派生类
  - `Quote base; Bulk_quote *bulkP = &base;` //错误! 无法将基类转换为派生类
- 在对象之间亦不存在类型转换
  - 将派生类赋值给基类是调用操作符函数,传入基类的引用作为参数, 因此非基类的成员会被**切掉(sliced down)**